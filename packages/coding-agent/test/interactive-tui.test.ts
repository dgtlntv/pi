...
describe("createInteractiveTui", () => {
	...
	it("shows the configured jump-to-bottom shortcut in the transcript indicator", async () => {
		initTheme("dark");
		const previousKeybindings = getKeybindings();
		const keybindings = new KeybindingsManager({ "tui.altScreen.bottom": "ctrl+j" });
		...
		if (!isViewportTUI(ui)) throw new Error("Expected fullscreen renderer");
		ui.setLayoutRoot(
