/**
 * Color math for generated themes: OKHSL <-> sRGB, perceptual contrast, and its inverse.
 *
 * OKHSL and Oklab are Björn Ottosson's color spaces. Perceptual contrast is a
 * lightness-contrast formula (0-108, directional: text on background) with an
 * optional low clip, so faint surfaces near the background stay measurable.
 */
...
/**
 * OKHSL lightness of a gray with relative luminance `y` (for grays, Oklab L = cbrt(Y)).
 * Exact for grays; saturated colors land a few percent off.
 */
...
const PERCEPTUAL = {
	...
	revBG: 0.65,
	blkThrs: 0.022,
	// biome-ignore lint/suspicious/noApproximativeNumericConstant: the formula's black clamp exponent, not sqrt(2)
	blkClmp: 1.414,
	loClip: 0.1,
	deltaYmin: 0.0005,
	scale: 1.14,
...
/** Screen luminance: a simple 2.4 gamma, before the black soft clamp. */
function screenLuminance(hex: string): number {
	const [r, g, b] = channels(hex);
	return 0.2126729 * r ** 2.4 + 0.7151522 * g ** 2.4 + 0.072175 * b ** 2.4;
}

/** Black soft clamp, which models flare on very dark colors. */
const clampBlack = (y: number): number =>
	y >= PERCEPTUAL.blkThrs ? y : y + (PERCEPTUAL.blkThrs - y) ** PERCEPTUAL.blkClmp;

/** Inverse of the black soft clamp. */
function unclampBlack(y: number): number {
	if (Number.isNaN(y) || y > PERCEPTUAL.blkThrs) return y;
	return (
		((y + 0.0387393816571401) * 1.9468554433171) ** (0.283343396420869 / PERCEPTUAL.blkClmp) / 1.9468554433171 -
		0.312865795870758
	);
}

/**
 * Signed perceptual contrast of text on a background: positive for dark text on light,
 * negative for light text on dark. The low clip reports |raw| < 0.1 as 0 and subtracts an
 * offset above it; `lowClip: false` returns the raw scaled contrast, for faint surfaces.
 */
...
/**
 * Relative luminance a gray needs to reach a perceptual contrast on a background.
 * @param minimum Absolute perceptual contrast to reach.
 * @param background Background color, `#rrggbb`.
 ...
 * @returns The luminance, or NaN when unreachable.
 */
export function targetLuminance(minimum: number, background: string, lighter: boolean, lowClip: boolean): number {
	const delta = (minimum / 100 + (lowClip ? PERCEPTUAL.loOffset : 0)) / PERCEPTUAL.scale;
	const y = clampBlack(screenLuminance(background));
	const screen = unclampBlack(
		lighter
			? (y ** PERCEPTUAL.revBG + delta) ** (1 / PERCEPTUAL.revTXT)
			: (y ** PERCEPTUAL.normBG - delta) ** (1 / PERCEPTUAL.normTXT),
	);
	// A gray's screen luminance is its channel ** 2.4; convert that channel to relative luminance.
