...
- **Footer** - Two stable rows: working directory/session details on the left and total token/cache usage (`↑` input, `↓` output, `R` cache read, `W` cache write, `CH` latest cache hit rate), cost, and context usage on the right; then transient working status on the left and the current model on the right. Totals include assistant responses, usage reported by tools, and summary generation.
