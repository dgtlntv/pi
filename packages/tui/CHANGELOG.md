...
- Added shared `SelectList` indicators that distinguish the configured value from keyboard focus, including `setCurrentValue()` and `renderSelectListIndicator()` for custom list renderers.
...
- Changed fullscreen scrollbars to render muted thin tracks with contrasting proportional two-cell-minimum thumbs, preserve underlying backgrounds without inheriting foreground styles from content or extended horizontal rules, reserve an unstyled column in `always` mode, reveal hidden `auto` tracks on pointer entry, expand the same-colored thumb on hover, and support track-click jumping in addition to thumb dragging.
